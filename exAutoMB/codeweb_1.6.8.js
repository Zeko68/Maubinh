...
window.sapBaiSoi2 = function () {
    ...
    for (let num = 0; num < 3; num++)
    {
        ...
        danhsachbaisapxep = [];
        danhsachbai.forEach(cardArray => {
            let cardLolArr = Object.assign({}, cardArray);
            // danhsachbaisapxep.push(mBaiSapXep(richy));
            // danhsachbaisapxep.push(__require("MBAI").default.getInstance().sapXep(richy));
            // let n = __require("MBAI").default.getInstance().sapXep2(cardArray);
            let n1 = mBaiSapXep(cardLolArr);
            let n = mBaiSapXep(cardArray);
            danhsachbaisapxep.push(n);
            // danhsachbaisapxep.push(__require("MBAI").default.getInstance().sapXep3(richy));
        });
        boxCardBinh = [];
        for (let i = 0; i < danhsachbaisapxep.length; i++) {
            ...
            for (let j = 0; j < 13; j++) {
                // zeda.push(danhsachbaisapxep[i].list[j].serverCode);
                zeda.push(danhsachbaisapxep[i][j].serverCode);
                if (j + 1 == 3) {
                    ingry.push(zeda);
                    zeda = [];
                }
...
window.setCurrentDupRecordRoom = function () {
    window.dupRoomID = document.getElementById('dupRoomID').value;
};
window.setCurrentDupRecordID = function (fromInput) {
    if (fromInput) {
...
if (!f12sct) {
    ...
    window.drawCardsMBSoi = function (boxPlayMBSoi, arrBoxCardBinh, boxChiId, boxId) {
        let box = document.getElementById(boxId);
        let impi = document.getElementById(boxPlayMBSoi);
        let auroralynn = document.getElementById(boxChiId);
        if (!auroralynn) {
            f12dC = document.createElement("div");
            f12dC.setAttribute("id", boxChiId);
            impi.append(f12dC);
        }

        auroralynn = document.getElementById(boxChiId);
        auroralynn.innerHTML = "";
        for (let i = 0; i < arrBoxCardBinh.length; i++) {
            auroralynn.innerHTML += draw1CardMB(i, arrBoxCardBinh[i], boxChiId);
        }
